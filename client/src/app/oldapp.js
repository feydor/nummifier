...
function toAQ(query) {
  ...
  while (reduction >= 10) {
    // keep reducing until we reach a single digit
...
function QueryBar(props) {
  ...
  function handleSubmit() {
    ...
    fetch(`/gematria`, {
      ...
      body: JSON.stringify({
        word: "" + props.query + "",
        reductions: toAQ(props.query).toString(),
      }),
    })
...
function DigitalReductionBars(props) {
  ...
  digitalReductionsAQ = digitalReductionsAQ.slice(
    0,
    digitalReductionsAQ.length - 2
  );
  ...
  return (
    <div id="DigitalReductionBars">
      ...
      {query.length === 0 ? (
        ...
        <div id="digitalReductionsAQ">{digitalReductionsAQ}</div>
      )}
    </div>
  );
}
